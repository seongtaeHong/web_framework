...
import stream_resources


def make_route_note(message, latitude, longitude):
    return stream_pb2.RouteNote(
        message=message,
        location=stream_pb2.Point(latitude=latitude, longitude=longitude))
...
def guide_get_feature(route_guide_stub):
    ...
    guide_get_one_feature(route_guide_stub,
                          stream_pb2.Point(latitude=0, longitude=0))


def guide_list_features(route_guide_stub):
    rectangle = stream_pb2.Rectangle(
        lo=stream_pb2.Point(latitude=400000000, longitude=-750000000),
        hi=stream_pb2.Point(latitude=420000000, longitude=-730000000))
...
def guide_record_route(route_guide_stub):
    feature_list = stream_resources.read_route_guide_database()
...
def run():
    ...
    with grpc.insecure_channel('localhost:50051') as channel:
        ...
        route_guide_stub = stream_pb2_grpc.RouteGuideStub(channel)
