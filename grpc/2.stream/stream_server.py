...
class _RouteGuideServicer(stream_pb2_grpc.RouteGuideServicer):
    """Provides methods that implement functionality of route guide server."""

    def __init__(self):
        self.db = stream_resources.read_route_guide_database()

    def GetFeature(self, request, context):
        feature = _get_feature(self.db, request)
        if feature is None:
            return stream_pb2.Feature(name="", location=request)
        else:
            return feature
    ...
    def RecordRoute(self, request_iterator, context):
        ...
        return stream_pb2.RouteSummary(
...
def serve():
    ...
    helloworld_pb2_grpc.add_GreeterServicer_to_server(_GreeterServicer(),
                                                      server)
    stream_pb2_grpc.add_RouteGuideServicer_to_server(
